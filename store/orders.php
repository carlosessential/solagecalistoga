 ...
 */
  ...
  if (zen_not_null($action) && $order_exists == true) {
	...
	$invoice_query = "
					...
					WHERE orders_id = " . $oID . "
					";
	$invoice_query_result = $db->Execute($invoice_query);
	$shift4_invoice_detail = ($invoice_query_result->RecordCount() > 0) ? $invoice_query_result->fields['shift4_invoice'] : '';  
    ...
    switch ($action) {
      ...
      case 'deleteconfirm':
		...
		if (isset($_POST['shift4'])) {
			if (isset($_POST['void-refund'])) {
				...
				if ($order->info['payment_module_code']) {
					if (file_exists(DIR_FS_CATALOG_MODULES . 'payment/' . $order->info['payment_module_code'] . '.php')) {
						...
						$module = new $order->info['payment_module_code'];						
						$amount_captured = $module->get_amount_captured($oID);						
						$amount_refunded = $module->get_amount_refunded($oID);
						$amount_not_refunded = number_format($amount_captured - $amount_refunded, 2);

						if ($amount_captured == 0){
							$module->_doVoid($oID, $_POST['void-refund-note']);
						} else {
							$invoice_xml = $module->admin_notification($oID, 'get invoice');
							if ($invoice_xml != "invoice not found") {
								if (number_format((float)$invoice_xml->primaryamount, 2) == $amount_captured && $_POST['refund_amount'] == number_format((float)$invoice_xml->primaryamount, 2)) {
									$module->_doVoid($oID, $_POST['void-refund-note'], $order->info['currency'], $amount_captured);								
								} else {
									if (isset($_POST['remove-order']) && $_POST['remove-order'] == 'yes') {
										if($amount_not_refunded > 0) {$module->_doRefund($oID, $_POST['void-refund-note'], $amount_not_refunded);}
									} else {
										$module->_doRefund($oID, $_POST['void-refund-note'], $_POST['refund_amount']);
									}
								}
							} else {
								if (isset($_POST['remove-order']) && $_POST['remove-order'] == 'yes') {
									$module->_doRefund($oID, $_POST['void-refund-note'], $amount_captured, '', false);	
								} elseif (isset($_POST['refund_amount']) && !empty($_POST['refund_amount'])) { 
									$module->_doRefund($oID, $_POST['void-refund-note'], $_POST['refund_amount'], '', false);
								}
							}
						}		
					}
			...
			if (isset($_POST['remove-order']) && $_POST['remove-order'] == 'yes') {zen_remove_order($oID, $_POST['restock']);}
			$parameters = ($_POST['shift4'] != 'list') ? "oID=$oID&action=edit" : zen_get_all_get_params(array('oID', 'action'));
			zen_redirect(zen_href_link(FILENAME_ORDERS, $parameters, 'NONSSL'));
			break;	
		} else {	
        ...
        if ($order->info['payment_module_code']) {
          if (file_exists(DIR_FS_CATALOG_MODULES . 'payment/' . $order->info['payment_module_code'] . '.php')) {
            ...
            if (method_exists($module, '_doCapt')) {
				$notes = (isset($_POST['shift4'])) ? '' : 'Complete';
				$module->_doCapt($oID, $notes, $order->info['total'], $order->info['currency']);
            }
          }
        }
        zen_redirect(zen_href_link(FILENAME_ORDERS, zen_get_all_get_params(array('action')) . 'action=edit', 'NONSSL'));
...
?>
      ...
      <tr>
        <td class="main"><strong><?php if (!empty($shift4_invoice_detail)) {echo "Shift4 Invoice: " . $shift4_invoice_detail; }?></strong></td>
      </tr>      
      <tr>
...
// Sort Listing
          switch ($_GET['list_order']) {
			...
			$shift4_invoice = $qry_shift4->RecordCount() == 0 ? '' : 'o.shift4_invoice,';											
?>
...
<?php if(!empty($shift4_invoice)){ ?>
				<td class="dataTableHeadingContent" align="center">Shift4<br />Invoice</td>
<?php }?>
...
//echo '<BR><BR>I SEE B: ' . $orders_query_raw . '<BR><BR>';
      ...
      $orders_query_raw = "select " . $search_distinct . $shift4_invoice . " o.orders_id, o.customers_id, o.customers_name, o.payment_method, o.shipping_method, o.date_purchased, o.last_modified, o.currency, o.currency_value, s.orders_status_name, ot.text as order_total" .
                          $new_fields . "
                          from (" . TABLE_ORDERS_STATUS . " s, " .
                          TABLE_ORDERS . " o " .
                          $new_table . ")
...
<?php if(!empty($shift4_invoice)){ ?>				
				<td class="dataTableContent" align="center"><?php echo $orders->fields['shift4_invoice']; ?></td>
<?php } ?>              
...
//      $contents[] = array('text' => TEXT_INFO_DELETE_INTRO . '<br /><br /><strong>' . $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname . '</strong>');
	  ...
	  if ($oInfo->payment_module_code == "shift4") {
	  $contents[] = array('text' => '<br />' . zen_draw_checkbox_field('void-refund', '', 'TRUE') . ' ' . 'Refund Shift4 transaction<br /><br />Note:<br />' . zen_draw_textarea_field('void-refund-note', 'soft', '100%', '3', 'Order deleted from Zen Cart.') . '<br />' . zen_draw_hidden_field('shift4', 'list') . '<br />' . zen_draw_hidden_field('remove-order', 'yes'));
	  }
