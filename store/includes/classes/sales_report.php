...
*/
  ...
  class sales_report {
    ...
    function sales_report($timeframe, $sd, $ed, $date_target, $date_status, $payment_method, $current_status, $manufacturer, $detail_level, $output_format, $order_total_validation) {
      global $db;
...
//      }
      ...
      if ($this->product_filter) $sql .= $this->product_filter . " \n";
      if ($this->customer_filter) $sql .= $this->customer_filter . " \n";
      $sql .= "ORDER BY o.cc_type ASC";
      ...
      if ($sales->RecordCount() > 0) {
        ...
        $this->grand_total['goods_tax'] += $this->timeframe[$id]['total']['goods_tax'];
        $this->grand_total['order_recorded_tax'] += $this->timeframe[$id]['total']['order_recorded_tax'];
    ...
    function build_li_totals($oID) {
      ...
      // Get the currency and recorded order total information
      $order_info_sql = "
        SELECT
          o.currency, o.currency_value, o.order_total
        FROM
          " . TABLE_ORDERS . " o
        WHERE
          o.orders_id = '" . $oID . "'";
        
      $order_info = $db->Execute($order_info_sql);
    ...
    function build_li_orders($oID, $field, $value) {
      ...
      if ($this->detail_level == 'order' || $this->detail_level == 'matrix') {
        ...
        if (!is_array($this->timeframe[$id]['orders'][$oID]) ) {
		  ...
		  $shift4_invoice = $c_data->fields['shift4_invoice'];
		  $shift4_unique_id = $c_data->fields['shift4_unique_id'];
		  $cc_type = $c_data->fields['cc_type'];
		  ...
		  $this->timeframe[$id]['orders'][$oID]['shift4_invoice'] = $shift4_invoice;
		  $this->timeframe[$id]['orders'][$oID]['shift4_unique_id'] = (!empty($shift4_unique_id)) ? '*****' . substr($shift4_unique_id, 0, 4) : '';
		  $this->timeframe[$id]['orders'][$oID]['cc_type'] = $cc_type;
        }
        ...
        foreach($tf['orders'] as $oID => $o_data) {		
          ...
          foreach($tf['matrix']['credit_cards'] as $key => $value) {		  
            ...
            if ($value['type'] == $cc_type) {
			  $_SESSION['key'] = $key;
			  $tf['matrix']['card_type_subtotals'][$cc_type] += $tf['orders'][$oID]['grand'];
			  $value['count']++;
    ...
    function output_csv($csv_header, $timeframe_sort, $li_sort_a, $li_sort_order_a, $li_sort_b, $li_sort_order_b) {
      $display_tax =  ($this->grand_total['goods_tax'] > 0 ? true : false);
